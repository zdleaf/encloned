...
void DB::openDB() {
  ...
  if (exitcode) {
    ...
    fs::permissions(DATABASE_LOCATION,
                    fs::perms::owner_read | fs::perms::owner_write);
  }

  int validDBcheck = execSQL("PRAGMA schema_version;");
  if (validDBcheck != 0) {
    cout << "DB: ERROR - unable to open index.db - it does not appear to be a "
            "valid database. Please check and resolve, or remove to create a "
            "new index"
         << endl;
...
void DB::closeDB() { sqlite3_close(db); }

sqlite3* DB::getDbPtr() { return db; }

const char* DB::getDbLocation() { return DATABASE_LOCATION; }
...
void DB::initialiseTables() {
  const char dirIndex[] =
      "CREATE TABLE IF NOT EXISTS dirIndex ("
  ...
  const char indexBackup[] =
      "CREATE TABLE IF NOT EXISTS indexBackup ("
      "PATH       TEXT    NOT NULL    UNIQUE,"
...
void DB::backupProgress(int leftToCopy, int totalToCopy) {
  cout << "DB: " << totalToCopy - leftToCopy << "/" << totalToCopy
       << " items backed up" << endl;
  cout.flush();
}
