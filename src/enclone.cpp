...
int main(int argc, char* argv[]) { enclone enc(argc, argv); }
...
int enclone::showOptions(const int& argc, char** const argv) {
  try {
    ...
    desc.add_options()("help,h", "display this help message\n")(
        "list,l", po::value<string>(),
        "show currently tracked/available files\n\n"
        "   local: \tshow all tracked local files\n"
        "   remote: \tshow all available remote files\n")(
        "add-watch,a", po::value<std::vector<string>>(&toAdd)->composing(),
        "add a watch to a given path (file or directory)")(
        "add-recursive,A",
        po::value<std::vector<string>>(&toRecAdd)->composing(),
        "recursively add a watch to a directory")(
        "del-watch,d", po::value<std::vector<string>>(&toDel)->composing(),
        "delete a watch from a given path (file or directory)")(
        "del-recursive,D", po::value<std::vector<string>>(&toDel)->composing(),
        "recursively delete all watches in a directory\n")(
        "restore,r", po::value<std::vector<string>>(&toRestore)->composing(),
        "restore files from remote\n\n"
        "   all: \trestore latest versions of all files\n"
        "   /path/to/file: \trestore the latest version of a file at specified "
        "path \n"
        "   filehash: \trestore a specific version of a file by providing the "
        "full hash as output by --list remote\n")(
        "target,t", po::value<string>(),
        "specify a target path to restore files to\n")(
        "restore-index,i", po::value<string>(),
        "restore an index/database from remote storage\n"
        "   show: \tshow all remotely backed up indexes\n"
        "   filehash: \trestore a specific index by giving the encrypted "
        "filename\n")("generate-key,k", "generate an encryption key")(
        "clean-up,c",
        "remove items from remote S3 which do not have a corresponding entry "
        "in fileIndex");
    ...
    // TODO: add more sets of conflicting options
    conflictingOptions(vm, "add-watch", "help");
    ...
    if (vm.count("list")) {
      ...
      if (arg == "local") {
        ...
        cout << "Incorrect argument to --list (-l) - enter either local or "
                "remote";
      }
    }
    ...
    if (vm.count("restore")) {
      ...
      if (vm.count("target")) {
        ...
        std::cerr
            << "error: please specify a --target (-t) directory to save to"
            << endl;
        return 1;
      }
    ...
    if (vm.count("restore-index")) {
      ...
      if (arg == "latest" || arg == "show" || arg.length() == 88) {
        ...
        std::cerr << "error: please enter 'latest', 'show' or an 88 character "
                     "encrypted index name"
                  << endl;
        return 1;
      }
    }
...
void enclone::conflictingOptions(const po::variables_map& vm, const char* opt1,
                                 const char* opt2) {
  if (vm.count(opt1) && !vm[opt1].defaulted() && vm.count(opt2) &&
      !vm[opt2].defaulted())
    throw std::logic_error(string("Conflicting options '") + opt1 + "' and '" +
                           opt2);
}
...
bool enclone::sendRequest(string request) {
  try {
    ...
    // read response
    // read_until requires a dynamic buffer to read into
    boost::asio::streambuf response;
    // all responses are suffixed with ";" - read until this delimiter
    asio::read_until(localSocket, response, ";");
    // get string response from buffer, remove delimiter
    std::string reply{buffers_begin(response.data()),
                      buffers_end(response.data()) - 1};
    cout << reply;
  } catch (std::exception& e) {
...
void enclone::generateKey() {
  ...
  if (fs::exists("key")) {
    cout << "Error: key already exists - please backup and remove old key "
